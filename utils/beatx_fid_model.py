...
def get_fid_model():
    ...
    ckpt_path = './datasets/BEAT_SMPL/beat_v2.0.0/beat_english_v2.0.0/weights/AESKConv_240_100.bin'
...
class VAESKConv(VAEConv):
    def __init__(self, args):
        ...
        smpl_fname = '/mnt/data/cbh/SynTalker/datasets/hub/smplx_models/smplx/SMPLX_NEUTRAL_2020.npz'
...
class VAESKConv(VAEConv):
    def __init__(self, args):
        super(VAESKConv, self).__init__(args)
        smpl_fname = './utils/smplx_models/smplx/SMPLX_NEUTRAL_2020.npz'
        smpl_data = np.load(smpl_fname, encoding='latin1')
        parents = smpl_data['kintree_table'][0].astype(np.int32)
        edges = build_edge_topology(parents)
        self.encoder = LocalEncoder(args, edges)
