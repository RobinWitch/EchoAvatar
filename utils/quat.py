...
def to_angleaxis(x, eps=1e-10):
    length = np.sqrt(np.sum(np.square(x[..., 1:]), axis=-1))
    angle = 2.0 * np.arctan2(length, x[..., 0])
    return angle[...,np.newaxis]*x[..., 1:] / (length[...,np.newaxis] + eps)
...
def from_angle_axis(angle_axis):

    # Calculate the magnitude (angle)
    angle = np.linalg.norm(angle_axis, axis=-1)[...,np.newaxis]
        
    # Normalize to get axis
    axis = angle_axis / angle
    
    # Convert to quaternion using the same formula as your original function
    c = np.cos(angle / 2.0)
    s = np.sin(angle / 2.0)
